...
type MerchantFilter = "ALL" | MerchantType;
...
export function AdminMerchantList({ merchants }: AdminMerchantListProps) {
  ...
  const filteredMerchants = useMemo(() => {
    ...
    return merchants.filter((merchant) => merchant.type === filter);
  }, [filter, merchants]);
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Pantau merchant kantin dan koperasi yang tersedia di SchoolCanteen.
          </p>
        </div>
      ...
      {filteredMerchants.length > 0 ? (
        <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {filteredMerchants.map((merchant) => {
            const productCount = merchant.productsCount ?? 0;

            const TypeIcon =
              merchant.type === "CANTEEN" ? UtensilsCrossed : ShoppingBasket;
